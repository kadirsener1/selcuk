...
def find_working_mactadi(start=70, end=90):
    print("🧭 mactadi domainleri taranıyor...")
    headers = {"User-Agent": "Mozilla/5.0"}

    for i in range(start, end + 1):
        url = f"https://www.mactadi{i}.xyz/"
        print(f"🔍 Taranıyor: {url}")
        try:
            response = requests.get(url, headers=headers, timeout=5)
            if response.status_code == 200 and "workers.dev/https://corestream.ronaldovurdu.help//hls/" in response.text:
...
def find_dynamic_player_domain(page_html):
    match = re.search(r'https?://(workers.dev/https://corestream.ronaldovurdu.help//hls)', page_html)
    if match:
...
# Ana işlem
html, referer_url = find_working_mactadi()
